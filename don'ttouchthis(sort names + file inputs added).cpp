...
bool isSortedDown(int arad[25])
{
  int i;
  for(i=0;i<24;i++)
  {
    if(arad[i]>arad[i+1]) return false;
  }
  return true;
}
bool isSortedUp(int araa[25])
{
  int i;
  for(i=0;i<24;i++)
  {
    if(araa[i]<araa[i+1]) return false;
  }
  return true;
}
